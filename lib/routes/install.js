...
var getAllGenerators = (function () {
  ...
  return function (cb) {
    if (allGenerators) return cb(allGenerators);

    var url = urls.npm.keyword('yeoman-generator');

    request({ url: url, json: true }, function (err, res, body) {
      if (err || res.statusCode !== 200) throw npmError(err + '\n' + body);
...
function searchMatchingGenerators(app, term, cb) {
  getAllGenerators(function (allGenerators) {
    cb(allGenerators.filter(function(generator){
      ...
      if (app.env.getGeneratorsMeta()[generator.name]) return false;
...
function isYeomanPackage(body) {
  if (body.author) {
    return body.author.name === 'The Yeoman Team';
  }
}
...
function fetchGeneratorInfo(generator, cb) {
  ...
  request({ url: url, json: true }, function (err, res, body) {
    if (err || res.statusCode !== 200) return cb(npmError(err + '\n' + body));

    var packageTypeSuffix = isYeomanPackage(body) ? chalk.green(' :{') : '';
    cb(null, {
      name: generator.name.replace(/^generator-/, '') + packageTypeSuffix,
...
function searchNpm(app, term) {
  searchMatchingGenerators(app, term, function (matches) {
    async.map(matches, fetchGeneratorInfo, function (err, choices) {
      if (err) throw err;
      promptInstallOptions(app, choices.sort(utils.alphaSort));
    });
  });
}
...
function installGenerator(app, pkgName) {
  ...
  return spawn('cnpm', ['install', '-g', pkgName], { stdio: 'inherit' })
